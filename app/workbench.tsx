...
type ConfirmDialog = { title: string; body: string; confirmLabel: string; onConfirm: () => void };
...
export function Workbench() {
  ...
  function ask(title: string, body: string, confirmLabel: string, onConfirm: () => void) { setDialog({ title, body, confirmLabel, onConfirm }); }
  ...
  function reset() { ask("重置全部 Demo 状态？", "当前角色、工单进度、时间线和列表筛选将恢复为初始值。", "确认重置", () => { localStorage.removeItem("hotel-workbench-v1"); setOrder(initialWorkOrder); setEvents(initialEvents); setCurrentUserId("lin"); setScenario("normal"); setView({ keyword: "", status: "", assignee: "all", from: "", to: "", page: 1 }); setToast("Demo 已恢复到待接收状态"); go("/dashboard"); }); }
  ...
  return <div className="app-shell">
    ...
    {dialog && <div className="modal-backdrop" role="presentation" onMouseDown={() => setDialog(null)}><section className="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="confirm-title" onMouseDown={e => e.stopPropagation()}><span className="dialog-symbol">?</span><h2 id="confirm-title">{dialog.title}</h2><p>{dialog.body}</p><div><button className="ghost" onClick={() => setDialog(null)}>取消</button><button className="primary" onClick={() => { const action = dialog.onConfirm; setDialog(null); action(); }}>{dialog.confirmLabel}</button></div></section></div>}
