...
type Ask = (testId: string, title: string, body: string, confirmLabel: string, action: () => boolean | void | Promise<boolean | void>) => void;
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <div className="page">
        ...
        {route.endsWith("/action") && <ActionPage order={order} user={user} perform={perform} go={go} ask={ask} recordError={scenario === "record_error"} />}
        {route.endsWith("/timeline") && <TimelinePageV2 order={order} user={user} events={events} perform={perform} go={go} ask={ask} />}
      </div>
    </main>
...
function ScenarioSelect({ value, onChange }: { value: Scenario; onChange: (s: Scenario) => void }) { return <label className="scenario">演示场景<select aria-label="选择演示场景" value={value} onChange={e => onChange(e.target.value as Scenario)}><option value="normal">正常主线</option><option value="duplicate">重复领取</option><option value="order_error">订单加载失败</option><option value="record_error">记录提交失败</option><option value="permission">权限不足</option><option value="conflict">状态已变化</option><option value="empty">列表无数据</option></select></label>; }
function Badge({ status }: { status: WorkOrderStatus }) { const s = statusMeta[status]; return <span className={`badge ${s.tone}`}><i />{s.label}</span>; }

function Dashboard({ order, go, openSharedPool }: { order: WorkOrder; go: (p: string) => void; openSharedPool: () => void }) { const cards = [["我的待接收", "3", "需及时响应", "amber"], ["我的进行中", "5", "2 条今日更新", "blue"], ["等待确认", "2", "等待业务确认", "violet"], ["共享待接收池", "12", "临近入住 4 条", "teal"]]; return <>
  <section className="hero"><div><span className="hero-tag">7月20日 · 国内客服一组</span><h2>早上好，林晓</h2><p>今天有 <b>4 条临近入住工单</b>需要优先关注，建议先处理共享池中的酒店确认事项。</p><button className="primary" onClick={openSharedPool}>查看共享待接收池 →</button></div><div className="hero-focus"><small>当前最新进展</small><b>酒店已返回确认号 HBR-731945</b><span>等待国内客服记录结果并提交确认</span><div className="progress"><i /></div><small>原始问题保留，订单展示状态已更新</small></div></section>
  ...
  <div className="metrics">{cards.map((c, i) => <button key={c[0]} className={`metric ${c[3]}`} onClick={i === 3 ? openSharedPool : () => go("/work-orders")}><span className="metric-icon">{["⌁","↗","✓","▤"][i]}</span><small>{c[0]}</small><strong>{c[1]}</strong><em>{c[2]}</em></button>)}</div>
...
function Detail({ order, user, go, perform, ask }: { order: WorkOrder; user: typeof users[number]; go: (p: string) => void; perform: (a: Action, c?: string) => boolean; ask: Ask }) {
...
function OrderPage({ order, go, error }: { order: WorkOrder; go: (p: string) => void; error: boolean }) {
  const fields = [["客户订单号","CL-NA-884217"],["客户名称","NorthStar Travel LLC"],["酒店","Harborline Riverside Hotel（海港滨河酒店）"],["城市","Boston, MA"],["房型","Deluxe King Room"],["入住人","Alex Morgan 等2人"],["供应商","Atlas Hotel Supply Ltd."],["供应商订单号","ATS-6928041"],["退改摘要","以订单原始条款为准；Demo 不执行修改或退款"]];
  return <><button className="back" onClick={() => go(base(order.id))}>← 返回工单详情</button><div className="status-separation"><div><small>国内客服工单状态</small><Badge status={order.status} /></div><span>独立生命周期，不自动联动</span><div><small>订单展示状态</small><b>酒店已确认，待客服回写</b></div></div>{error ? <section className="panel error-state"><span>!</span><h3>订单信息暂时加载失败</h3><p>这不会影响工单查看和处理记录。请稍后重试。</p><button className="secondary">重新加载</button></section> : <section className="panel body-panel"><div className="panel-head"><div><span className="eyebrow">ORDER CONTEXT</span><h3>ORD-20260718-58321</h3></div><span className="assumption">Demo 假设字段</span></div><div className="order-highlight"><div><small>入住日期</small><b>2026-07-22</b></div><span>→</span><div><small>离店日期</small><b>2026-07-25</b></div><div><small>入住时长</small><b>3 晚 · 2 间</b></div><div><small>酒店确认号</small><b className="confirm-no">HBR-731945</b></div></div><div className="info-grid">{fields.map((x,i) => <div className={i === 8 ? "span-2" : ""} key={x[0]}><small>{x[0]} {![0,5,7,8].includes(i) && <em>假设</em>}</small><b>{x[1]}</b></div>)}</div></section>}<div className="footer-actions"><button className="secondary" onClick={() => go(`${base(order.id)}/related`)}>查看关联任务与联系 →</button></div></>;
}
...
function ActionPage({ order, user, perform, go, ask, recordError }: { order: WorkOrder; user: typeof users[number]; perform: (a: Action, c?: string) => boolean; go: (p: string) => void; ask: Ask; recordError: boolean }) {
  const [content, setContent] = useState(""); const [progress, setProgress] = useState(""); const [summary, setSummary] = useState(finalResult); const [errors, setErrors] = useState<string[]>([]);
  function record(e: FormEvent) { e.preventDefault(); const err = [!content && "处理内容", !progress && "处理结果/当前进展"].filter(Boolean) as string[]; setErrors(err); if (err.length) return; if (recordError) { setErrors(["模拟提交失败：输入已保留，请切换正常主线后重试"]); return; } if (perform("record", `${content}；${progress}`)) { setContent(""); setProgress(""); } }
  function confirm(message: string) {
    if (message.includes("提交处理结果")) ask("confirm-submit-for-review", "提交处理结果待确认？", "提交后主工单进入待确认；订单、海外工单和授权任务不会改变。", "确认提交", () => { const ok = perform("submit", summary); if (ok) go(`${base(order.id)}/timeline`); return ok; });
    else ask("confirm-accept-work-order", "接收这张工单？", "接收后工单进入进行中，当前处理人将变为林晓。", "确认接收", () => perform("accept"));
    return false;
  }
...
function TimelinePageV2({ order, user, events, perform, go, ask }: { order: WorkOrder; user: typeof users[number]; events: TimelineEvent[]; perform: (a: Action, c?: string) => boolean; go: (p: string) => void; ask: Ask }) {
  const [filter, setFilter] = useState("all");
  const [opinion, setOpinion] = useState("");
  const shown = filter === "all" ? events : events.filter(event => event.type === filter);
  const decide = (action: "confirm" | "return") => ask(
    action === "confirm" ? "confirm-close-work-order" : "confirm-return-work-order",
    action === "confirm" ? "确认办结这张工单？" : "退回继续处理？",
    action === "confirm" ? "办结后不可继续编辑，最终结果和全部历史将保留。" : "退回后恢复为进行中，林晓仍为处理人，原因将写入时间线。",
    action === "confirm" ? "确认办结" : "确认退回",
    () => perform(action, opinion),
  );
  return <><button className="back" onClick={() => go(base(order.id))}>← 返回工单详情</button>{order.status === "pending_confirmation" && <section className="confirmation-banner"><div><span className="eyebrow">RESULT CONFIRMATION</span><h2>处理结果等待确认</h2><p>请确认客服提交的结果是否已回答原始需求。</p></div><div><Badge status={order.status} /><span>确认人：陈岚</span></div></section>}{order.finalResult && <section className="completed-banner"><span>✓</span><div><small>最终处理结果</small><p>{order.finalResult}</p></div></section>}<div className="timeline-layout"><section className="panel timeline-panel"><div className="panel-head"><div><span className="eyebrow">FACT TIMELINE</span><h3>事实时间线</h3></div><span>{shown.length} 条事件 · 倒序</span></div><div className="timeline-filters">{[["all","全部"],["order","订单"],["work_order","国内工单"],["overseas","海外工单"],["authorization","授权"],["contact","联系记录"]].map(item => <button className={filter === item[0] ? "active" : ""} key={item[0]} onClick={() => setFilter(item[0])}>{item[1]}</button>)}</div><div className="events">{shown.map(event => <div className={event.outcome === "failure" ? "event failed" : "event"} key={event.id}><time>{event.time.split(" ")[1]}<small>{event.time.split(" ")[0]}</small></time><i /><div><span className={`object-tag ${event.type}`}>{eventLabel(event.type)}</span><h4>{event.action}</h4><p>{event.content}</p><small>{event.actor} · {event.result}</small></div></div>)}</div></section><aside>{order.status === "pending_confirmation" ? <section className="panel confirm-panel"><span className="eyebrow">DECISION</span><h3>确认处理结果</h3><div className="result-box"><small>客服提交摘要</small><p>{order.finalResult || finalResult}</p></div><label>确认意见 / 退回原因 <b>*</b><textarea aria-label="确认意见或退回原因" value={opinion} onChange={e => setOpinion(e.target.value)} placeholder="填写确认意见或需补充的内容…" /></label>{can("confirm", order, user) ? <><button className="primary wide" disabled={!opinion} onClick={() => decide("confirm")}>✓ 确认并办结</button><button className="danger wide" disabled={!opinion} onClick={() => decide("return")}>↩ 退回继续处理</button></> : <div className="permission"><b>当前用户不能确认</b><p>请切换至指定确认人陈岚或业务主管赵凯。处理人不能自审。</p></div>}</section> : <section className="panel next-panel"><h3>{order.status === "completed" ? "工单已办结" : "尚未提交确认"}</h3><p>{order.status === "completed" ? "所有关键动作已记录，不可编辑或删除。" : "请先由当前处理人填写最终摘要并提交待确认。"}</p>{order.status === "in_progress" && <button className="primary wide" onClick={() => go(`${base(order.id)}/action`)}>进入处理动作</button>}</section>}</aside></div></>;
}
